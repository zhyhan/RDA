...
def evaluate(model_instance, input_loader):
    ...
    for i in range(num_iter):
        ...
        feature, _, probabilities = model_instance.predict(inputs)
...
def train(model_instance, train_source_loader, test_target_loader, group_ratios, max_iter, optimizer, lr_scheduler, eval_interval):
    ...
    while True:
        ...
        if iter_num > max_iter:
            #np.save('statistic/ResNet_feature_target.npy', all_feature.cpu().numpy())
            break
    print('finish train')
...
if __name__ == '__main__':
    ...
    lr_scheduler = INVScheduler(gamma=cfg.lr_scheduler.gamma,
                                ...
                                init_lr=cfg.init_lr)
    to_dump = train(model_instance, train_source_loader, test_target_loader, group_ratios, max_iter=100000, optimizer=optimizer, lr_scheduler=lr_scheduler, eval_interval=10000)
